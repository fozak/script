...
globalThis.CW.Schema["Task"] = {
  ...
  _state: {
    1: {
      ...
      sideEffects: {
        "0_1":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; var updateRun = await run_doc.child({ operation: 'update', target_doctype: 'Task', query: { where: { name: rec.name } }, input: { status: 'Pending' }, options: { render: false, internal: true } }); }",
        "1_2":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; var updateRun = await run_doc.child({ operation: 'update', target_doctype: 'Task', query: { where: { name: rec.name } }, input: { status: 'Approved' }, options: { render: false, internal: true } }); }",
        "1_3":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; var updateRun = await run_doc.child({ operation: 'update', target_doctype: 'Task', query: { where: { name: rec.name } }, input: { status: 'Rejected' }, options: { render: false, internal: true } }); }",
        "2_3":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; var updateRun = await run_doc.child({ operation: 'update', target_doctype: 'Task', query: { where: { name: rec.name } }, input: { status: 'Rejected' }, options: { render: false, internal: true } }); }",
        "3_1":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; var updateRun = await run_doc.child({ operation: 'update', target_doctype: 'Task', query: { where: { name: rec.name } }, input: { status: 'Pending' }, options: { render: false, internal: true } }); }",
      ...
      rules: {
        "1_2":
          "(run_doc) => { const doc = run_doc.target?.data?.[0] || {}; return CW._getDimValue(doc, '0', CW._getStateDef(run_doc.target_doctype)['0']) === 0; }",
      },
      requires: {},
    },
  },
  ...
  fields: [
    ...
    {
      depends_on:
        'eval:doc.status == "Closed" || doc.status == "Pending Review"',
  ...
  permissions: [
    {
      ...
      create: 1,
      delete: 1,
      transitions: { "1.0_1": "Submit for Review", "1.3_1": "Resubmit" },
    },
    {
...
globalThis.CW.Schema["User"] = {
  ...
  fields: [
    ...
    {
      depends_on:
        "eval:in_list(['System User', 'Website User'], doc.user_type) && doc.enabled == 1",
      fieldname: "sb1",
      fieldtype: "Section Break",
      label: "Roles",
      permlevel: 1,
    ...
    { fieldname: "location", fieldtype: "Data", label: "Location", no_copy: 1 },
    {
    ...
    { fieldname: "column_break_22", fieldtype: "Column Break" },
    { fieldname: "interest", fieldtype: "Small Text", label: "Interests" },
    { fieldname: "bio", fieldtype: "Small Text", label: "Bio", no_copy: 1 },
    {
      default: "0",
    ...
    },
    {
      collapsible: 1,
      depends_on:
        "eval:doc.enabled && (!doc.__islocal || !cint(doc.send_welcome_email))",
    ...
    { fieldname: "column_break_65", fieldtype: "Column Break" },
    {
    ...
    { fieldname: "column_break_11", fieldtype: "Column Break" },
    { fieldname: "column_break_26", fieldtype: "Column Break" },
    { fieldname: "section_break_63", fieldtype: "Column Break" },
    {
    ...
    { fieldname: "column_break_75", fieldtype: "Column Break" },
    {
      default: "0",
      depends_on: "eval:(doc.document_follow_notify== 1)",
    ...
    { fieldname: "settings_tab", fieldtype: "Tab Break", label: "Settings" },
    {
    ...
    {
      description: "rotating it invalidates all active sessions",
      fieldname: "tokenKey",
      fieldtype: "Data",
      hidden: 1,
      label: "Token Key",
    ...
    },
    {
      default: 0,
      description:
        "Controls whether the user's email is publicly visible (0 = Hidden, 1 = Visible)",
      fieldname: "emailVisibility",
      fieldtype: "Check",
      label: "Email Visible",
    },
    {
      description: "virtual for token",
      fieldname: "token",
      fieldtype: "Data",
      is_virtual: 1,
      label: "Token",
    ...
    {
      default: 0,
      description: "Set to 1 by Adapter.auth.activate after code verification",
      fieldname: "verified",
      fieldtype: "Check",
      hidden: 1,
      label: "Verified",
  ...
  links: [
    { group: "Profile", link_doctype: "Contact", link_fieldname: "user" },
    { group: "Profile", link_doctype: "Blogger", link_fieldname: "user" },
    { group: "Logs", link_doctype: "Access Log", link_fieldname: "user" },
    { group: "Logs", link_doctype: "Activity Log", link_fieldname: "user" },
    { group: "Logs", link_doctype: "Energy Point Log", link_fieldname: "user" },
    { group: "Logs", link_doctype: "Route History", link_fieldname: "user" },
    {
      group: "Settings",
    ...
    { group: "Activity", link_doctype: "ToDo", link_fieldname: "allocated_to" },
    {
      group: "Integrations",
  ...
  _state: {
    1: {
      ...
      transitions: { 0: [1, 4], 1: [2, 3, 4], 2: [1, 4], 3: [1, 4], 4: [0] },
      labels: {
        "0_1": "Activate",
      ...
      sideEffects: {
        "0_1":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; await pb.collection('users').update(rec.name, { verified: true }); }",
        "1_2":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; await pb.collection('users').update(rec.name, { verified: false }); }",
        "1_4":
          "async function(run_doc) { var rec = run_doc.target && run_doc.target.data && run_doc.target.data[0]; if (!rec) return; await pb.collection('users').update(rec.name, { verified: false }); }",
      },
      rules: {},
  ...
  permissions: [
    {
      ...
      transitions: {
        "1.0_1": "Activate User",
        "1.0_4": "Cancel Invitation",
        "1.1_2": "Lock Account",
        "1.1_3": "Require Password Reset",
        "1.1_4": "Disable User",
        "1.2_1": "Unlock Account",
        "1.3_1": "Complete Reset",
        "1.4_0": "Re-invite",
      },
    },
    { role: "User Manager", read: 1, write: 1, transitions: {} },
  ],
...
`,
  ...
  userFields: [
    ...
    },
    {
      ...
      in_local_view: 1,
      onCreate: (run_doc) => {
        run_doc.target.data[0].auth_status = 1;
      },
    },
    {
      ...
      in_local_view: 1,
      onCreate: (run_doc) => {
        run_doc.target.data[0].email_status = 0;
      },
    },
    {
      fieldname: "owner",

...
(function () {
  ...
  const cfg = Object.assign({
    ...
    fields: [
      { name: 'first_name',     reqd: 1, label: 'First name',       icon: 'user',           type: 'text',     placeholder: 'First name',                    col2: 1 },
      { name: 'last_name',      reqd: 1, label: 'Last name',        icon: 'user',           type: 'text',     placeholder: 'Last name',                     col2: 1 },
      { name: 'email',          reqd: 1, label: 'Email',            icon: 'mail',           type: 'email',    placeholder: 'you@company.com',               col2: 1 },
      { name: 'phone',                   label: 'Phone',            icon: 'phone',          type: 'tel',      placeholder: '+1 (617) 000-0000',             col2: 1 },
      { name: 'linkedin_url',   reqd: 1, label: 'LinkedIn profile', icon: 'brand-linkedin', type: 'url',      placeholder: 'https://linkedin.com/in/yourname' },
      { name: 'recommended_by',          label: 'Recommended by',  icon: 'user-check',     type: 'text',     placeholder: 'Name of NESEN member who recommended you',
        hint: 'Two recommendations significantly improve your application.' },
      { name: 'interest',       reqd: 1, label: 'I am interested in…', type: 'textarea',   rows: 5,
        placeholder: 'Tell us what brought you to NESEN and what you are hoping to build, find, or contribute here.',
      ...
      { name: 'terms_accepted', reqd: 1, label: 'Terms accepted',   type: 'checkbox',
        checkLabel: 'I have read and agree to the <a href="/terms.html">Terms of Use</a> and <a href="/privacy.html">Privacy Policy</a>' },
    ]
  ...
  const full   = cfg.fields.filter(f => !f.col2 && f.type !== 'hidden' && f.type !== 'checkbox')
  ...
  document.getElementById('_va-submit').addEventListener('click', async () => {
    ...
    cfg.fields.forEach(f => {
      ...
      if (!el) return
      collected[f.name] = f.type === 'checkbox' ? (el.checked ? 1 : 0) : el.value.trim()
    })

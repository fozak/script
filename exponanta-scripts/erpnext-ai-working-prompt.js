(function() {
  // Widget configuration and data AI saving to custom_code instead of text field
  const WIDGET_CONFIG = {
      "data": {
            "settings": {
                  "erpUrl": "https://apfile.com",
                  "aiProvider": "openai",
                  "aiApiKey": "",
                  "lastQuery": "Show me all items, do NOT include query: stock_qty"
            },
            "history": [],
            "favorites": [
...
frappe.call({
          ...
          const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
              ...
              messages: [
                { 
                  role: "system", 
                  content: `You are an ERPNext/Frappe Framework code assistant. Generate ONLY JavaScript code that uses frappe.call() to interact with ERPNext API based on user queries. No COMMENTS. 
                  ...
                  - Return ONLY pure executable JavaScript code with NO comments, markdown, or explanations
  ...
  try {
    // Find the target container (usually near the custom_code field)
    const targetContainer = document.querySelector('[data-fieldname="custom_code"]')?.closest('.frappe-control');

(function () {
  ...
  // ── Note bubble ──────────────────────────────────────────────────────────────
  function makeBubble(id, getNote, setNote, getPos) {
    ...
    bubble.onclick = e => {
      e.stopPropagation();
      const n = prompt('Edit note (blank = delete):', getNote());
      if (n === null) return;
  ...
  function highlightRange(range, id, note) {
    ...
    mark.dataset.ann = id;
    mark.style.cssText = 'background:rgba(255,220,0,0.45);border-radius:2px;cursor:pointer;';
    try { range.surroundContents(mark); } catch(_) { return false; }

    const { bubble, cleanup } = makeBubble(
      id,
      ()  => data.find(a => a.id === id)?.note || '',
      n   => { const a = data.find(a => a.id === id); if (a) { a.note = n; save(); } },
      ()  => { const r = mark.getBoundingClientRect(); return [r.left, r.bottom + 6]; }
    );
  ...
  function placePin(el, ann) {
    ...
    pin.style.cssText = PIN_CSS + `
      ...
      background:#e53935;color:#fff;font:bold 13px system-ui;
    ...
    const { bubble, cleanup: bubbleCleanup } = makeBubble(
      ann.id,
      ()  => data.find(a => a.id === ann.id)?.note || '',
      n   => { const a = data.find(a => a.id === ann.id); if (a) { a.note = n; save(); } },
      ()  => { const [x, y] = getPos(); return [x + 14, y + 14]; }
    );
    bubble.style.display = 'none';

   pin.onclick = e => {
      e.stopPropagation();
      const current = data.find(a => a.id === ann.id);
      if (!current) return;
      const n = prompt('Edit note (blank = delete):', current.note);
      if (n === null) return;
  ...
  document.addEventListener('mouseup', e => {
    ...
    // TEXT — takes priority if there is a selection
    ...
    if (sel && !sel.isCollapsed && sel.toString().trim().length >= 2) {
      ...
      const ann = {
        ...
        selector: bestSelector(container.nodeType === 3 ? container.parentElement : container)
      };
      data.push(ann); save();

...
// ── setup ────────────────────────────────────────────────────────────────────
if (CW._editor._glyphDisposable) CW._editor._glyphDisposable.dispose();
if (CW._editor._hoverDisposable) CW._editor._hoverDisposable.dispose();
...
CW._editor._hoverDisposable = CW._editor.onMouseMove(e => {
  if (!e.target.position) return;
  const line = e.target.position.lineNumber;
  CW._editor._decorations = CW._editor.deltaDecorations(CW._editor._decorations || [], [{
    range: new monaco.Range(line, 1, line, 1),
    options: { glyphMarginClassName: 'cw-step-glyph' }
  }]);
});

CW._editor._glyphDisposable = CW._editor.onMouseDown(e => {
  if (e.target.type !== monaco.editor.MouseTargetType.GUTTER_GLYPH_MARGIN) return;
  CW._editor._glyphLine = e.target.position.lineNumber;
  CW._editor.runCode();
});

console.log('glyphs ready');
...
// step: tasks
run.tasks = await run.notebook.child({
  operation:      'select',
  target_doctype: 'Task',
...
// step: console
run.console = await (async () => { console.log('i am fired4'); return true; })();

// step: update_tasks
run.update_tasks = await Promise.all(
...
// step: tasks
run.tasks2 = await run.notebook.child({
  operation:      'select',
  target_doctype: 'Task',

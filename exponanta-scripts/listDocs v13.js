...
* Single function that handles ALL navigation
*/ 
...
pb.navigate = {
  ...
  to: function(path, options = {}) {
    // Path can be object or string
    if (typeof path === 'object') {
      path = this.buildPath(path);
    }
    
    // Use your router (React Router or hash-based)
    if (options.newTab) {
  ...
  // Build path from object (prevents typos and inconsistency)
  buildPath: function(params) {
    const { view, doctype, name, reportName } = params;
    ...
    switch(view) {
      ...
      case 'form':
        return `/${doctype}/${name}`;
      case 'report':
        return `/${doctype}/report/${reportName}`;
      case 'dashboard':
        return `/${doctype}/dashboard`;
      default:
        return '/';
    }
  },
  
  // Convenience methods (so you don't repeat params)
  toList: function(doctype) {
    return this.to({ view: 'list', doctype });
  },
  
  toForm: function(doctype, name, options) {
    return this.to({ view: 'form', doctype, name }, options);
  },
  
  toReport: function(doctype, reportName) {
    return this.to({ view: 'report', doctype, reportName });
  },
  
  toNew: function(doctype) {
    return this.to({ view: 'form', doctype, name: 'new' });
  },
  
  // Get path without navigating (for href attributes)
...
/* pb.components.DocLink added 
*
*/

pb.components.DocLink = function({ doctype, name, children, newTab = false }) {
  const { createElement: e } = React;
  
  const href = pb.navigate.getPath({ view: 'form', doctype, name });
  
  return e('a', {
    href: href,
    onClick: (ev) => {
      ev.preventDefault();
      pb.navigate.toForm(doctype, name, { newTab });
    },
    onAuxClick: (ev) => {
      // Middle mouse button = new tab
      if (ev.button === 1) {
        ev.preventDefault();
        pb.navigate.toForm(doctype, name, { newTab: true });
      }
    },
    className: pb.BS.text.primary
  }, children);

 ...
 * @returns {Promise<Array|Object>} Records array or {data, meta} object
 */
...
//changed react component Main grid
    ...
    async function fetchData() {
      try {
        ...
        const [records, schemaData] = await Promise.all([
          pb.listDocs(doctype, {}, { view: 'list' }), // Already returns filtered fields!
          pb.getSchema(doctype) // Only for field metadata (labels, types) - IF we re
        ]);
        setData(records);

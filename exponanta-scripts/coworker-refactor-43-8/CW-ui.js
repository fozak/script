...
function _getMainRuns() {
  return Object.values(CW.runs)
    .filter(
      (r) => r.component?.startsWith("Main") && r.options?.render !== false,
    )
...
function _getBreadcrumbs() {
  ...
  if (current.component === "MainGrid")
    return [
      ...
      {
        text: current.source_doctype || current.target_doctype || "List",
        runName: null,
      },
    ];
  ...
  if (el)
    el.innerHTML = _getBreadcrumbs()
      .map((c, i, arr) => {
        ...
        if (c.runName)
          return `<li class="breadcrumb-item"><a href="#" onclick="navigateTo('${c.runName}');return false;">${c.text}</a></li>`;
...
if (field.fieldtype === "Datetime") {
  ...
  return ce("input", {
    ...
    value: display,
    readOnly: readOnly,
    onChange: readOnly ? undefined : (e) => onChange(new Date(e.target.value).getTime()),
    onBlur: readOnly ? undefined : (e) => onBlur(new Date(e.target.value).getTime()),
  })
}
...
if (readOnly)
  return ce("input", {
    type: "text",
    className: "form-control",
    value: typeof localVal === "object" ? JSON.stringify(localVal) : String(localVal ?? ""),
    readOnly: true,
  })
  ...
  if (field.fieldtype === "Table") {
    ...
    const keys = colFields
      .map((f) => f.fieldname)
      .filter((k) => k in (childData[0] || {}));
    return ce(
      "div",
      ...
      ce(
        "table",
        {
          className: "table table-vcenter table-hover table-sm card-table mb-0",
        },
        ce(
          "thead",
          {},
          ce(
            ...
            keys.map((k) =>
              ce(
                "th",
                { key: k },
                colFields.find((f) => f.fieldname === k)?.label || k,
              ),
            ),
          ),
        ),

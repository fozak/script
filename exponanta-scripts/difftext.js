///added from http://incaseofstairs.com/jsdiff/diff.js
...
const logDifferences = () => {
    ...
    timeoutId = setTimeout(() => {
        ...
        if (currentInnerText !== previousInnerText) {
            ...
            // Use jsdiff to find differences
            const diff = Diff.diffWords(previousInnerText, currentInnerText);

            // Log only the added parts
            let addedText = '';
            diff.forEach(part => {
                if (part.added) {
                    addedText += part.value; // Collect all added parts
                }
            });

            // Log the added text if any
            if (addedText) {
...
const eventListenerOptions = { capture: true }; // Optional: use capture phase if needed
window.addEventListener('scroll', logDifferences, eventListenerOptions);
document.addEventListener('input', logDifferences, eventListenerOptions);  // Captures changes from keyboard actions
document.addEventListener('click', logDifferences, eventListenerOptions);   // Captures clicks that might change text
document.addEventListener('blur', logDifferences, eventListenerOptions);    // Captures loss of focus to log the final text c
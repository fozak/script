  ...
  const coworkerRun = {
    ...
    install: function (coworker) {
      ...
      coworker._resolveAll = function (op) {
        ...
        // Resolve operation (aliases map to: select, create, update, delete)
        resolved.operation =
          opAliases[op.operation?.toLowerCase()] || op.operation;

        // Resolve source/target doctypes using semantic logic
        ...
        resolved.source_doctype = source_raw
          ? dtAliases[source_raw?.toLowerCase()] || source_raw
          : null;
        resolved.target_doctype = target_raw
          ? dtAliases[target_raw?.toLowerCase()] || target_raw
          : null;

        // Resolve other fields via config
        for (const resolverName in this._config) {
          if (
            resolverName.startsWith("_resolve") &&
            resolverName !== "_resolveOperation" &&
            resolverName !== "_resolveDoctype"
          ) {
            ...
            if (inputValue !== undefined) {
              ...
              resolved[resolver.outputField] =
                mapping[inputValue?.toLowerCase()] || inputValue;
            }
          }
        }
      ...
      // After: const resolved = this._resolveAll(op);
      // Add this:

      // Auto-infer view from operation if rendering requested
      if (op.options?.render === true && !op.view && !op.component) {
        const operationToView = {
          select: "list",
          create: "form",
          update: "form",
          delete: null,
        };
        resolved.view = operationToView[resolved.operation] || null;
      }

      // Resolve component and container from view
      if (resolved.view) {
        const compMapping = this._config?._resolveComponent?.mapping || {};
        const contMapping = this._config?._resolveContainer?.mapping || {};

        resolved.component = op.component || compMapping[resolved.view] || null;
        resolved.container = op.container || contMapping[resolved.view] || null;
      }
      // ============================================================

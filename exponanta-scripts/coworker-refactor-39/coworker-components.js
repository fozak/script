...
const MainForm = ({ run }) => {
    ...
    return {
      ...
      onButtonClick: (fieldname, value) => {
        ...
        if (config.debounce > 0) {
          ...
          timersRef.current[`onButtonClick_${fieldname}`] = setTimeout(perform, config.debounce);
        } else {
          perform();
        }
      },
  ...
  const implementedTypes = [
    ...
    "Text Editor", "Code", "HTML", "Section Break",
    "Column Break", "Tab Break", "Button", "Attach Image",
  ];
    ...
    React.createElement(
      ...
      fields
        ...
        .map((field) => {
          ...
          // ✅ Layout fields (full width) — React.createElement, not direct call
          if (fieldType.layoutOnly && fieldType.render) {
            return React.createElement(
              ...
              React.createElement(fieldType.render, { field, handlers, run }),  // ✅
            );
          }
          ...
          return React.createElement(
            ...
            // ✅ customComponent — React.createElement, not direct call
            fieldType.customComponent && fieldType.render
              ? React.createElement(fieldType.render, {
                  key: field.fieldname,
                  field,
                  value: safeValue,
                  handlers,

...
(function () {
  ...
  useEffect(() => {
    pb.query("All", {}, { includeSchema: false })
      .then(result => {
    ...
    // ✅ Get unique doctypes from "All" view (cached globally)
    useEffect(() => {
      // If cached, reuse
      ...
      pb.query("All", {}, { includeSchema: false })
        .then((result) => {
          const uniqueDoctypes = [
            ...new Set(result.data.map((item) => item.doctype).filter(Boolean)),
          ].sort();
    ...
    const performSearch = async (text) => {
      ...
      try {
        const searchPromises = doctypes.map(async (doctype) => {
          try {
            const result = await pb.query(doctype, {
  ...
  const App = function () {
    ...
    useEffect(() => {
      const unsubscribe = pb.navigation.subscribe((list, loading) => {
        ...
        if (list && list.data) {
          const isSingleItem =
            list.data.length === 1 && list.params?.query?.take === 1;

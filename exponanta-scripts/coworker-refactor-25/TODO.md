...
javascriptvalidate(run) {
  return this._validate(run.doc, run.output?.schema);
}
...
// In save():
const merged = { ...original, ...delta };  // original from run.output + delta from run.input
// Then validates run.doc which is: { ...run.output.data[0], ...run.input }
...
Solution: Fetch schema ONLY in controller, handlers trust run_doc.output.schema:
...
if (opConfig.requiresSchema && !options.skipSchema) {
  ...
  if (doctype && doctype !== "Schema") {
    if (!run_doc.output) run_doc.output = {};
    run_doc.output.schema = await coworker.getSchema(doctype);
  }
}

// In handlers - NEVER fetch, just use
const schema = run_doc.output?.schema;  // ✅ Trust controller
...
run.output.data = [saveRun.output.data[0]];  // ✅ Mutates run.output
